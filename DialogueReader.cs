...
namespace VNS
{
    public class DialogueReader : Popup
    {
        ...
        public async void NextPhrase()
        {
            ...
            Texture img = (Texture)GD.Load(menuManager.SelectedScenePath + "/characters/" + currentSpeaker + speakerMod + speakerEmotion + ".png");
            ...
            previousSpeaker = characterManagers[previousIndex].Speaker;
            previousPosition = characterManagers[previousIndex].Position;
            phraseNum += 1;
            finished = true;
            //continue if autoplaying or if fast forwarding
            if (autoplayManager.Auto || fastManager.FastPressed)
            {
                NextPhrase();
            }
            return;
